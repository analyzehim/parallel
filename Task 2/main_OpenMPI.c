...
// The value of product (r(k),r(k)) is calculating ...
        ...
        double local_sp = 0.0;
        #pragma omp parallel for reduction(+:local_sp) schedule(static)
		for(i=x1; i <= x2; i++)
            for(j=y1; j <= y2; j++)
...
// The value of product sp = (Ar(k),r(k)) is calculating ...
		...
		for(i=x1; i <= x2; i++)
            for(j=y1; j <= y2; j++)
				local_sp += LeftPart(ResVect,i,j)*ResVect[NX*j+i]*hx*hy;

        MPI_Barrier(Grid_Comm);
            sp = 0.0;
        MPI_Allreduce(&local_sp, &sumSp, 1, MPI_DOUBLE, MPI_SUM, Grid_Comm);
		tau = tau/sp;
...
// sp == (Ar(k-1),r(k-1)) == (Ag(0),g(0)), k=1.
	...
	// The value of product (Ar(k),g(k-1)) is calculating ...
        ...
        #pragma omp parallel for reduction(+:local_alpha) collapse(2) schedule(static)
		for(i=x1; i <= x2; i++)
            for(j=y1; j <= y2; j++)
				local_alpha += LeftPart(ResVect,i,j)*BasisVect[NX*j+i]*hx*hy;
	...
	// The value of product (r(k),g(k)) is being calculated ...
        ...
        #pragma omp parallel for reduction(+:local_tau) collapse(2) schedule(static)
		for(i=x1; i <= x2; i++)
            for(j=y1; j <= y2; j++)
				local_tau += ResVect[NX*j+i]*BasisVect[NX*j+i]*hx*hy;

        MPI_Barrier(Grid_Comm);
        tau = 0.0;
        MPI_Allreduce(&local_tau, &tau, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	...
	// The value of product sp = (Ag(k),g(k)) is being calculated ...
        ...
        double local_sp = 0.0;
        #pragma omp parallel for reduction(+:local_sp) collapse(2) schedule(static)
		for(i=x1; i <= x2; i++)
            for(j=y1; j <= y2; j++)
				local_sp += LeftPart(BasisVect,i,j)*BasisVect[NX*j+i]*hx*hy;
